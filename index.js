...
const Categories = {
    0 : {'bmi_category' : 'NA', "health_risk" : 'NA', "bmi" : -1},
...
async function getBmiDetails(bmi){
    ...
    index = await d3.bisect(ranges, bmi, 0, 12); //? 0: start index 12 size of the array Ranges which is declared above constants
    ...
    Categories[index/2 + 1]['bmi'] = bmi

    return Categories[index/2 + 1]
...
async function processJsonData(data){
    ...
    for(let record of data){
        ...
        if(!record.HeightCm || !record.WeightKg){
            ...
            record = Object.assign(record, bmi_category_details);
